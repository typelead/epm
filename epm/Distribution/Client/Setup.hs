...
import Distribution.Utils.NubList (NubList, toNubList, fromNubList)
...
import Distribution.Simple.Setup
       (ConfigFlags(..), BuildFlags(..), ReplFlags, TestFlags(..),
        BenchmarkFlags(..), SDistFlags(..), HaddockFlags(..),
        readPackageDbList, showPackageDbList, Flag(..), toFlag, fromFlag,
        flagToMaybe, flagToList, optionVerbosity, boolOpt, boolOpt',
        trueArg, falseArg, optionNumJobs)
import Distribution.Simple.InstallDirs
       (PathTemplate, InstallDirs(sysconfdir), toPathTemplate,
        fromPathTemplate)
...
import Distribution.Simple.Utils (wrapText, wrapLine)
import Data.Char (isSpace, isAlphaNum)
import Data.List (intercalate, delete, deleteFirstsBy)
...
globalCommand commands =
    CommandUI
    { commandName = ""
    , commandSynopsis = "Command line interface to EPM (ETA Package Manager)."
    , commandUsage = \pname ->
                          "See https://github.com/typelead/epm/tree/1.22 for more information.\n" ++
                          "\n" ++
                          "Usage: " ++
                          pname ++ " [GLOBAL FLAGS] [COMMAND [FLAGS]]\n"
    , commandDescription = Just $
      \pname ->
           let commands' =
                   commands ++ [commandAddAction helpCommandUI undefined]
               ...
               par = ""
               addCmd n =
                   case lookup n cmdDescs of
                       Nothing -> ""
                       Just d -> "  " ++ align n ++ "    " ++ d
               addCmdCustom n d =
                   case lookup n cmdDescs    -- make sure that the
                         of
...
filterConfigureFlags flags cabalLibVersion
  ...
  where
    ...
    flags_1_20_0 =
        ...
        ,
          -- HACK: See #2409.
          configProgramPaths = ("cabalConfProf", "/TRUE") `delete`
...
defaultConfigExFlags =
    mempty
    { configSolver = Flag defaultSolver
...
configureExOptions _showOrParseArgs =
    ...
    , option
          ...
          (optArg
               allowNewerArgument
               (fmap Flag allowNewerParser)
               (Flag AllowNewerAll)
               allowNewerPrinter)]
  where
    allowNewerArgument = "DEPS"
...
-- ------------------------------------------------------------
replCommand
    :: CommandUI (ReplFlags, BuildExFlags)
replCommand =
    parent
    { commandDefaultFlags = (commandDefaultFlags parent, mempty)
    , commandOptions = \showOrParseArgs ->
...
-- ------------------------------------------------------------
testCommand
    :: CommandUI (TestFlags, BuildFlags, BuildExFlags)
testCommand =
    parent
    { commandDefaultFlags = ( commandDefaultFlags parent
                            , Cabal.defaultBuildFlags
                            , mempty)
    , commandOptions = \showOrParseArgs ->
                            liftOptions
                                get1
...
-- ------------------------------------------------------------
benchmarkCommand
    :: CommandUI (BenchmarkFlags, BuildFlags, BuildExFlags)
benchmarkCommand =
    parent
    { commandDefaultFlags = ( commandDefaultFlags parent
                            , Cabal.defaultBuildFlags
...
freezeCommand =
    ...
    , commandNotes = Nothing
    , commandUsage = usageAlternatives "freeze" ["", "PACKAGES"]
    ...
    , commandOptions = \showOrParseArgs ->
                            ...
                            optionSolverFlags
                                ...
                                (\v flags ->
                                      flags
                                      { freezeMaxBackjumps = v
                                      })
                                freezeReorderGoals
                                (\v flags ->
                                      flags
                                      { freezeReorderGoals = v
                                      })
                                freezeIndependentGoals
                                (\v flags ->
                                      flags
                                      { freezeIndependentGoals = v
                                      })
                                freezeShadowPkgs
                                (\v flags ->
                                      flags
         ...
         case s of
             ...
             _ ->
                 case readPToMaybe pkgsParser s of
                     ...
                     Nothing ->
                         Left ("Cannot parse the list of packages: " ++ s)
  where
...
installCommand =
    ...
    , commandNotes = Just $
      \pname ->
           ...
           "  " ++
           pname ++
           " install 'foo < 2'       " ++ "    Constrained package version\n"
    , commandDefaultFlags = (mempty, mempty, mempty, mempty)
    , commandOptions = \showOrParseArgs ->
                            liftOptions
                                get1
...
installOptions showOrParseArgs =
    ...
    optionSolverFlags
        ...
        (\v flags ->
              flags
              { installMaxBackjumps = v
              })
        installReorderGoals
        (\v flags ->
              flags
              { installReorderGoals = v
              })
        installIndependentGoals
        (\v flags ->
              flags
              { installIndependentGoals = v
              })
        installShadowPkgs
        (\v flags ->
              flags
    ...
    , option
          ...
          (\v flags ->
                flags
                { installSymlinkBinDir = v
                })
          (reqArgFlag "DIR")
    , option
      []
      ["eta-patches-directory"]
      "Specify explicit Eta patches directory"
      installEtaPatchesDirectory
      (\v flags ->
           flags
           { installEtaPatchesDirectory = v
           })
      (reqArgFlag "DIR")
    , option
          []
          ...
          (\v flags ->
                flags
                { installSummaryFile = v
                })
          (reqArg'
               "TEMPLATE"
               (\x ->
                     toNubList [toPathTemplate x])
    ...
    , option
          ...
          (\v flags ->
                flags
                { installReportPlanningFailure = v
                })
          trueArg
    , option
          []
    ...
    case showOrParseArgs         -- TODO: remove when "cabal install"
                                 -- avoids
          of
        ParseArgs ->
            [ option
...
defaultInitFlags =
    emptyInitFlags
    { IT.initVerbosity = toFlag normal
    }
    ...
    , commandOptions = \_ ->
                            ...
                            , option
                                  ...
                                  (reqArg
                                       "VERSION_RANGE"
                                       (readP_to_E
                                            ("Cannot parse Cabal version range: " ++)
                                            (toFlag `fmap` parse))
                                       (flagToList . fmap display))
                            , option
                                  ['l']
                            ...
                            , option
                                  ...
                                  (reqArg'
                                       ...
                                       (\s ->
                                             toFlag $
                                             maybe (Left s) Right (readMaybe s))
...
win32SelfUpgradeCommand =
    ...
    , commandOptions = \_ ->
                            [ optionVerbosity
                                  win32SelfUpgradeVerbosity
                                  (\v flags ->
                                        flags
                                        { win32SelfUpgradeVerbosity = v
                                        })]
    }
      ...
      \pname ->
           concat
               ...
               , paragraph $
                 ...
                 pname ++ " commands, e.g. `repl`, `build`, `exec`."
               , paragraph $
                 "Currently, " ++
                 pname ++
               ...
               , headLine "add-source:"
               , indentParagraph $
                 "Make one or more local package available in the" ++
...
optionSolver get set =
    option
        ...
        ("Select dependency solver to use (default: " ++
         ...
         "). Choices: " ++
         allSolvers ++
         ", where 'choose' chooses between 'topdown' and 'modular' based on compiler version.")
        get
        set
        (reqArg
             "SOLVER"
...
parsePackageArgs
    :: [String] -> Either String [Dependency]
...
indentParagraph = unlines . map (("  " ++) . unwords) . wrapLine 77 . words
